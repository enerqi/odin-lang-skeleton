...
# Every recipe carries a `[group('...')]` so `just --list` stays readable as the file grows - an
# ungrouped one lists on its own above the groups, so group any recipe you add. `just --groups` prints
# the names. bench/bench.just files its recipes under `perf` alongside the hyperfine ones.
