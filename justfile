...
#
# The directories are created all at once rather than one per line because just starts a new shell per
# recipe line, so five lines would pay for five shell launches. That was the whole cost back when
# Windows recipes ran under PowerShell (hyperfine: ~149ms to launch, against ~40ms for the actual
# directory creation); under cmd a launch is ~9ms, so batching saves ~36ms rather than ~600ms. Still
# the right shape, no longer dramatic. The corollary is unchanged either way: scoping this to only the
# one directory a given recipe needs saves ~5ms of that 40 and is not worth the complexity.
# ---
