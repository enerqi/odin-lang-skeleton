...
main :: proc() { 	// Operational setup before calling `main_program`
	// `os.exit` sets the process exit code but terminates immediately, skipping deferred cleanup. Defer it
	// at the very top so it runs LAST (defers are LIFO) -- after every teardown defer below. Odin evaluates
	// a deferred call's arguments at scope exit (not at the defer site, unlike Go), so this reads the final
	// `exit_code` that `main_program` assigns.
	exit_code: int
