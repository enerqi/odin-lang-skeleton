...
main :: proc() { 	// Operational setup before calling `main_program`
	exit_code: int
	defer os.exit(exit_code) // deferred first so it runs last
	defer restore_console_codepage() // see CONSOLE_UTF8
...
___________________________________________________________________________________________________________________
	...
	- Build with `-define:CONSOLE_UTF8=false` to leave the Windows console codepage alone. On by
	  default on Windows only, where it is what makes non-ASCII output legible (see below)
___________________________________________________________________________________________________________________
*/
